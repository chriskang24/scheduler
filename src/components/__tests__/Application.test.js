...
import { render, cleanup, waitForElement, fireEvent, getByText, prettyDOM, getAllByTestId, getByAltText, getByPlaceholderText, queryByText, queryByAltText, wait, getByDisplayValue } from "@testing-library/react";
...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Check that the interviewer to be edited is shown.

    expect(getByDisplayValue(appointment, "Archie Cohen")).toBeInTheDocument();

    fireEvent.change(getByDisplayValue(appointment, "Archie Cohen"), {
      target: { value: "Sly Jones" }
    });

    fireEvent.click(queryByAltText(appointment, "Sylvia Palmer"))

    expect(getByDisplayValue(appointment, "Sly Jones")).toBeInTheDocument();
    expect(getByText(appointment, "Sylvia Palmer")).toBeInTheDocument();

    // 5. Click the "Save" button on the the screen.
    ...
    // 7. Wait until the element with the edited name text "Sly Jones" is displayed.

    await waitForElement(() => getByText(appointment, "Sly Jones"));
